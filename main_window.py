...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _get_export_data(self) -> tuple[list[str], list[list], list[int], list[int], list[int], int]:
        """Видимые столбцы и строки. Возвращает (headers, rows, project_ids, numeric_cols, percent_cols, name_col)."""
        if not hasattr(self, "table") or self.table.columnCount() != 12:
            return [], [], [], [], [], 0
        numeric_logical = {3, 4, 5, 6, 7, 8}
        header = self.table.horizontalHeader()
        headers = []
        logical_cols = []
        numeric_col_indexes = []
        ...
        for visual in range(12):
            ...
            if self.table.isColumnHidden(logical):
                continue
            export_idx = len(headers)
            headers.append(self.TABLE_HEADERS[logical])
            logical_cols.append(logical)
            if logical == 0:
                name_col_index = export_idx
            elif logical in numeric_logical:
                numeric_col_indexes.append(export_idx)
            elif logical == 9:
                percent_col_indexes.append(export_idx)
        visible_rows = [r for r in range(self.table.rowCount()) if not self.table.isRowHidden(r)]
        if not visible_rows:
            return headers, [], [], numeric_col_indexes, percent_col_indexes, name_col_index
        ...
        for r in sorted_row_indices:
            ...
            for logical in logical_cols:
                ...
                if logical == 10:
                    ...
                    raw = item.data(QtCore.Qt.ItemDataRole.UserRole) if item else None
                    val = float(raw) if raw is not None and raw != -float("inf") else None
                elif logical == 9:
                    raw = item.data(QtCore.Qt.ItemDataRole.UserRole) if item else None
                    val = float(raw) / 100.0 if raw is not None and raw != -float("inf") else None
                else:
                    val = (item.text() or "").strip()
                row_data.append(val)
            rows.append(row_data)
            name_item = self.table.item(r, 0)
            pid = name_item.data(QtCore.Qt.ItemDataRole.UserRole) if name_item else None
            project_ids.append(int(pid) if pid is not None else 0)
        return headers, rows, project_ids, numeric_col_indexes, percent_col_indexes, name_col_index
    ...
    def on_export_excel(self):
        ...
        if not path:
            return
        try:
            headers, rows, project_ids, numeric_cols, percent_cols, name_col = self._get_export_data()
            out = export_excel.export_table_to_excel(
                path, headers, rows, project_ids,
                numeric_col_indexes=numeric_cols,
                percent_col_indexes=percent_cols,
