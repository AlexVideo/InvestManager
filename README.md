...
**Версия 0.6.0** — локальный учёт инвест-проектов, маркетинга, договоров, корректировок, ревизий, а также учёт договоров и актов по услугам и работам.
...
4. **Дальше**: запускайте **push_to_github.bat** — он сделает коммит (с версией из `version.py`), создаст тег версии (например **v0.6.0**) и отправит всё на GitHub.

5. **Откат к предыдущей версии**: на GitHub в репозитории откройте **Code** → выпадающий список веток → **Tags** → выберите нужную версию (например `v0.5.1`). Или локально: `git fetch --tags`, затем `git checkout v0.5.1` (после работы — `git checkout main` чтобы вернуться к последней версии).
