...
class MarketingDialog(QtWidgets.QDialog):
    def __init__(self, project_id: int, parent=None, record_id: int | None = None, prefill: dict | None = None):
        super().__init__(parent)
        self.project_id = project_id
        self.record_id = record_id
        self.setWindowTitle("Маркетинговое заключение" + (" — редактирование" if record_id else ""))
        ...
        file_row.addWidget(self.file_open_btn)
        form.addRow("Файл (опц.):", file_row)
    ...
    def _load(self):
        r = db.get_marketing(self.record_id)
        if not r: return
    ...
    def _apply_prefill(self, prefill: dict):
        """Предзаполнение из последнего маркетинга (сумма, дата, файл, примечание)."""
    ...
    def pick_file(self):
        path, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Выбрать файл")
    ...
    def on_accept(self):
        ...
        # Копируем файл в папку Files/marketing рядом с базой (если выбран внешний файл)
        if file_path:
            ...
            if os.path.isfile(abs_path):
                ...
                if not abs_path_n.startswith(project_dir):
                    try:
                        file_path = db.copy_attachment_to_files(abs_path, "marketing", self.project_id)
                    except Exception as e:
        ...
        if self.record_id:
            db.update_marketing(self.record_id, amt, date, file_path, note)
        else:
            db.record_marketing(self.project_id, amt, date, file_path, note)
        self.accept()
